...
def parse_peb(emu, addr):
    return {"ldr_data": ucutils.parse_ptr(emu, addr + 0x18)}
...
def emit_ldr_data_table_entry(
    ...
    ucwin.init_list_entry(emu, addr + 0x00)  # load order
    ucwin.init_list_entry(emu, addr + 0x10)  # mem order
    ucwin.init_list_entry(emu, addr + 0x20)  # init order
