...
def parse_peb(emu, peb_addr):
    return {
        'ldr_data': ucutils.parse_ptr(emu, peb_addr+OFFSET_LDR_DATA)
    }
...
def emit_ldr_data_table_entry(emu, ldr_addr,
                              dllbase_addr,
                              entrypoint_addr,
                              sizeofimage,
                              fulldllname_us,
    ...
    init_list_entry(emu, ldr_addr+0x00)  # load order
    init_list_entry(emu, ldr_addr+0x08)  # mem order
    init_list_entry(emu, ldr_addr+0x10)  # init order
